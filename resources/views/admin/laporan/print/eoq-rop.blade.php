@php
    ...
    $laporanService = app(LaporanEoqRopService::class);
@endphp
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Cetak — Laporan Analisis EOQ & ROP</title>
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .header { margin-bottom: 14px; font-family: 'Times New Roman', Times, serif; color: #0f172a; }
        .header__layout {
            display: flex;
            align-items: center;
            justify-content: center;
        ...
        .header__logo { flex: 0 0 120px; width: 120px; }
        .header__logo img {
            display: block;
            width: 100%;
            height: auto;
        ...
        .header__body { flex: 0 1 auto; max-width: calc(100% - 142px); text-align: center; }
        .header__brand {
            margin: 0 0 4px;
            font-size: 14pt;
            font-weight: 700;
        ...
        .header__address { margin: 0; font-size: 8.5pt; line-height: 1.35; }
        .header__social-row {
            display: flex;
            align-items: center;
            justify-content: space-between;
        ...
        .status-aman { color: #047857; font-weight: 700; }
        .status-restock { color: #b91c1c; font-weight: 700; }
        .ringkasan {
            margin-top: 12px;
            padding: 8px 10px;
        ...
        .ringkasan td { padding: 3px 0; font-size: 8.5pt; }
        ...
        .ringkasan--inline p { margin: 0 0 4px; font-size: 9pt; }
        .ringkasan--inline p:last-child { margin-bottom: 0; }
        .signature {
            margin-top: 28px;
            padding-top: 8px;
            display: flex;
        ...
        .signature__place-date { margin: 0 0 6px; font-size: 10pt; }
        .signature__signatory {
            display: inline-flex;
            flex-direction: column;
            align-items: center;
        }
        .signature__role { margin: 0; font-size: 10pt; align-self: flex-start; }
        .signature__space { height: 56px; width: 100%; }
        .signature__name {
            margin: 0;
            font-size: 10.5pt;
            font-weight: 700;
...
<body>
    ...
    <div class="print-page">
        ...
        <h2 class="report-title">Laporan Analisis EOQ & ROP</h2>

        <table class="data">
            <thead>
                <tr>
                    <th style="width:5%;">No</th>
                    <th style="width:12%;">Kode Onderdil</th>
                    <th style="width:28%;">Nama Onderdil</th>
                    <th style="width:10%;">EOQ</th>
            ...
            <tbody>
                @forelse ($analisis as $index => $row)
                    @php
                        $liveStatus = $laporanService->resolveStatus(
                            (int) $row->stok_saat_ini,
                            (int) $row->hasil_rop,
                        );
                        $statusClass = $liveStatus === LaporanEoqRopService::STATUS_AMAN
                            ? 'status-aman'
                            : 'status-restock';
                    @endphp
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $row->kode_onderdil }}</td>
                        <td class="text-left">{{ $row->nama_onderdil }}</td>
                        <td>{{ number_format($row->hasil_eoq, 0, ',', '.') }}</td>
                        <td>{{ number_format($row->hasil_rop, 0, ',', '.') }}</td>
                        <td>{{ number_format($row->stok_saat_ini, 0, ',', '.') }}</td>
                        <td class="{{ $statusClass }}">{{ $liveStatus }}</td>
                    </tr>
                @empty
                    <tr>
        ...
        <div class="ringkasan ringkasan--inline">
            <p><strong>Total Onderdil Dianalisis</strong> : {{ number_format($ringkasan['total_onderdil'], 0, ',', '.') }}</p>
            <p><strong>Total Kondisi Aman</strong> : {{ number_format($ringkasan['total_aman'], 0, ',', '.') }}</p>
            <p><strong>Total Perlu Restock</strong> : {{ number_format($ringkasan['total_perlu_restock'], 0, ',', '.') }}</p>
        </div>
