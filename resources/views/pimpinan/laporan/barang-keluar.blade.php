    ...
    <div class="space-y-6">
        <div class="admin-section-card flex flex-col gap-4 p-5 sm:flex-row sm:items-start sm:justify-between sm:p-6">
            <div class="min-w-0">
                ...
                <h2 class="mt-1 text-xl font-bold text-slate-900 sm:text-2xl">Barang Keluar</h2>
                <p class="mt-1 text-sm text-slate-500">Rekap transaksi barang keluar onderdil. Dapat difilter dan dicetak ke PDF.</p>
            ...
            <a
                href="{{ route('pimpinan.laporan.keluar.pdf', $printQuery) }}"
                target="_blank"
                rel="noopener"
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
            ...
            <div class="laporan-stat-card laporan-stat-card--aman">
                <p class="laporan-stat-card__label">Total Unit Keluar</p>
            ...
            <div class="laporan-stat-card laporan-stat-card--menipis">
                <p class="laporan-stat-card__label">Total Jenis Onderdil</p>
                <p class="laporan-stat-card__value">{{ number_format($ringkasan['total_jenis_onderdil'], 0, ',', '.') }}</p>
            </div>
        </div>

        <div class="admin-table-card admin-table-card--dropdown-safe">
            <div class="admin-table-toolbar admin-table-toolbar--filters">
                <form method="GET" action="{{ route('pimpinan.laporan.keluar') }}" class="flex flex-col gap-3 xl:flex-row xl:items-end">
                    <div class="flex-1">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @if ($hasFilter)
                            <a
                                href="{{ route('pimpinan.laporan.keluar') }}"
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr>
                            ...
                            <th class="{{ $thClass }}">Tanggal Keluar</th>
                            <th class="{{ $thClass }}">Kode Onderdil</th>
                            <th class="{{ $thClass }} text-left">Nama Onderdil</th>
                            <th class="{{ $thClass }}">Jumlah Keluar</th>
                            <th class="{{ $thClass }} text-left">Keterangan</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white">
                        @forelse ($barangKeluars as $keluar)
                            <tr class="transition hover:bg-rose-50/30">
                                ...
                                <td class="{{ $tdLabelClass }}">{{ $keluar->tanggal_keluar->format('d-m-Y') }}</td>
                                <td class="{{ $tdLabelClass }}">{{ $keluar->onderdil?->kode_onderdil ?? '-' }}</td>
                                <td class="{{ $tdClass }} text-left">
                                    <span class="cell-truncate" title="{{ $keluar->onderdil?->nama_onderdil }}">{{ $keluar->onderdil?->nama_onderdil ?? '-' }}</span>
                                </td>
                                <td class="{{ $tdLabelClass }}">{{ number_format($keluar->jumlah, 0, ',', '.') }}</td>
                                <td class="{{ $tdClass }} text-left">
                                    <span class="cell-truncate" title="{{ $keluar->keterangan }}">{{ $keluar->keterangan ?: '-' }}</span>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-12 text-center text-slate-500">
                                    Tidak ada data barang keluar yang sesuai filter.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
